...
class GameState {
  ...
  /**
   * Switch to a different playlist without invalidating existing player links.
   * Updates playlistSongs and playlistId in-place; does not change gameId or
   * card IDs.  Card grids must be regenerated by the caller after this call.
   * @param {Array<Object>} songs  Tracks from the new playlist.
   * @param {string}        pid   Spotify playlist ID.
   * @throws {Error} if the game is currently active.
   */
  updatePlaylist(songs, pid) {
    if (this.status === 'active') {
      throw new Error('Cannot change playlist while a game is active. End the game first.');
    }
