...
This app is a persistent **Node.js server with WebSockets**, so it needs a host that supports long-running processes — [Render](https://render.com) is used here (the same category of host as the batting-order-generator's Netlify, just for server-side apps instead of static sites).
...
Render's free tier spins the service down after 15 minutes of inactivity. The first request after a cold start may take ~30 seconds. To avoid this, upgrade to a paid plan or use Render's "Health Check" ping service.
